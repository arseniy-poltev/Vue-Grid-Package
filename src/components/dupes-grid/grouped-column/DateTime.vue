      ...
      <template v-slot:activator="{ on }">
        ...
        >
          <v-edit-dialog
            @save="save"
          >
            {{ isEdited ? (newValue | formatDT) : (text | formatDT) }}
      ...
      >
        ...
        >
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title>{{ beforeValue | formatDT }}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <v-list-item>
            <v-list-item-avatar
...
<script>
  ...
  export default {
    name: 'VdgGridGroupedColumnDateTime',
    components: {
    },
    props: {
      field: {
    ...
    computed: {
      ...
      css () {
        let self = this
        let css = 'text-no-wrap text-center'
    ...
    methods: {
      save () {
        let self = this
        self.isEdited = true
        self.$emit('datetime-edited', self.groupItems[0][self.groupField], self.field, self.oldValue, self.newValue)
      },
    },
  }
</script>
